...
** Les fichiers de l'agenda
   #+begin_src emacs-lisp
     ...
     (defconst org-agenda-files-cours
       (cl-list* "~/org/cours.org" "~/org/edt.org" "~/org/agenda-pronote.org"
...
#+begin_src emacs-lisp
  ...
  (defun org-caldav-async ()
    ...
    (interactive)
    (require 'org-caldav)
    (if org-caldav-calendars
        (dolist (calendar org-caldav-calendars)
          (dolist (file (or (plist-get calendar :files)) org-caldav-files)
    ...
    (async-start
     ...
     (lambda ()
       (let ((org-caldav-delete-org-entries 'always)
             (load-path (cl-list* "~/.emacs.d/src/"
                                  "~/.emacs.d/src/org-caldav/"
                                  load-path))
             (debug-on-error t))
         (require 'org-caldav)
         (load "~/.emacs.d/modes/after-load-org.org")
         (eiu      (uraet))
         (load "~/.emacs.d/modes/after-load-org-caldav.org.el")
         (org-caldav-sync)))
...
** Lisons les jours et mois français
   ...
   #+begin_src emacs-lisp
     ...
     ;;         ;; problem with parsing. Probably should wrote a advice for org-mode-ask-date
     ;;         ;; (--filter (not (string= "mar" (car it))) parse-time-months
     ;;         (nconc  parse-time-months  ; mar is for mardi, not march
     ;;                 parse-time-months-french))
     ;;   (push '((4) parse-time-months) parse-time-rules)
     ;;   )
   #+end_src
...
#+name: note-tri
#+begin_src emacs-lisp
  (defun copy-note-here (ask)
    (interactive "P")
    (save-excursion
      (unless (and (not ask) (boundp 'note-for-org-trimestre))
        (set (make-local-variable 'note-for-org-trimestre) (read-from-minibuffer "trimestre: ")))
      (unless (and (not ask) (boundp 'note-for-org-table-name))
...
#+begin_src emacs-lisp
  (defun copy-apreciation-here (ask)
    (interactive "P")
    (save-excursion
      (let* ((start (point-marker)))
        (outline-previous-visible-heading 1)
        (looking-at "\*+ \\(.*\\)")
        (let* ((name  (match-string-no-properties 1))
               (regex-name (format "\"%s\"" (regexp-quote name)))
               beg end
               names maxs notes)
          (goto-char (point-min))
          ...
          (forward-line 1)
          (setq end (point))
          (setq line (split-string (buffer-substring-no-properties beg end) "\t"))
          (goto-char start)
          (org-indent-line)
          (insert "Moy: " (nth 2 line) " remarques: " (nth 3 line))))))
